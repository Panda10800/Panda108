...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "A_A_5":
      try:
...
@Client.on_message(filters.command(["المطور باندا","باندا","المطور"], ""))
async def deev(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="L_Y_Y_Y")
...
@Client.on_message(filters.command(["باندا","المطور باندا","المبرمج","المطور ","المبرمج كوماندا"], ""))
async def sultan(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="A_A_5")
     name = user.first_name
     username = user.username 
     bio = user.bio
     user_id = user.id
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["A_A_5"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["L_Y_Y_Y"]:
   ...
   if message.from_user.username == "A_A_5":
...
async def tiillllli(client, message):
 if message.from_user.username in ["A_A_5"]:
   await client.send_sticker(message.chat.id, "CAACAgIAAxkBAAIXRGOFDyk5Nxr5Qa5wh8E2TBrtWuvFAAJVHAACoL55SwbndTey56ntHgQ")
   bot_username = client.me.username
   user = await get_userbot(bot_username)
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**♪ 𝙾𝚆𝙽𝙴𝚁 ➧ @L_Y_Y_Y  💎 .**")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**♪ 𝙾𝚆𝙽𝙴𝚁 ➧ @A_A_5  💎 .**")
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="**♪ 𝙾𝚆𝙽𝙴𝚁 ➧ @L_Y_Y_Y  💎 .**")
...
async def qurann(client, message):
  ...
  await message.reply_video(video=video, caption="**♪ 𝙾𝚆𝙽𝙴𝚁 ➧ @A_A_5  💎 .**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in OWNER :
         ...
         return
     if userr.username in ["A_A_5"]:
         await message.reply_text("**♪ رتبتك هي : المطور كوماندا  💎 .**")
         return
     if userr.username in ["L_Y_Y_Y"]:
         await message.reply_text("**♪ رتبتك هي : المطور باندا 💎 .**")
         return
     if userr.id == dev:
